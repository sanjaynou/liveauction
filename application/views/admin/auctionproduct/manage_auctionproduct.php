        ...
        <div class="col-xs-12">
          ...
          <div class="box">
            <div class="box-header">
              <h3 class="box-title">Manage Auction Product</h3>
			  <a href="<?php echo base_url(); ?>admin/auctionproduct/add" class="btn btn-primary add-photo"  style="color:#ffffff; float:right;"><i class="fa fa-plus">&nbsp;</i>Add New</a>
            </div>
            <!-- /.box-header -->
            <div class="box-body table-responsive">
             ...
             <?php
			 $sn=1;
			 foreach($auctionproduct as $allauctionproduct)
			 {?>
                <tr>
                  <td><?php echo $sn; ?></td>
                  <td><?php echo substr(strip_tags($allauctionproduct->product_name),0,20); if(strlen($allauctionproduct->product_name)>20){echo ".....";}?></td>
				  <td><?php 
				  $seller_id = $allauctionproduct->seller_id;
				$seller=$this->seller_model->get_seller($seller_id);
				 if(!empty($seller)){ echo ucfirst($seller->first_name).'&nbsp;'.ucfirst($seller->last_name); }?></td>
				 
				 <td>
				 <?php 
				  /*$category_id = $allauctionproduct->category_id;
				$category=$this->category_model->get_category($category_id);
				 if(!empty($category)){ echo ucfirst($category->category_name); }*/
				  echo $allauctionproduct->category_path;
				 ?></td>
				  
				 
				  			
				  				  
   <td>  <a href="<?php echo base_url(); ?>admin/auctionproduct/view/id/<?php echo $allauctionproduct->product_id;?>" title="View  Detail" class="btn btn-xs btn-warning"><i class="fa fa-eye"></i></a>
				  <a href="<?php echo base_url(); ?>admin/auctionproduct/edit/id/<?php echo $allauctionproduct->product_id;?>" title="Update Product" class="btn btn-xs btn-primary"><i class="fa fa-pencil"></i></a>
<a href="javascript:void(0);" onClick="deleteRecord(<?php echo $allauctionproduct->product_id;?>)"  title="Delete Buyer" class="btn btn-xs btn-danger"><i class="fa fa-trash-o"></i></a>
...
if(confirm('Do you want to delete ? ')){
	...
	document.location.href="<?php echo base_url(); ?>admin/auctionproduct/delete/id/"+x;
